...
- **What may be relied on: [docs/STABILITY.md](docs/STABILITY.md)** — the public API and the
...
- **New public functions** need a golden LaTeX sample in `tests/test_latex_coverage.py`, and new
  mathematical types need one in `TYPE_SAMPLES` there; both tests fail otherwise. They also
  need adding to `PRELUDE_NAMES` in `tests/test_stability.py`, deliberately: that list *is*
  the public API (docs/STABILITY.md), and from 1.0 removing a name costs a deprecation cycle.
...
make site                        # render the documentation site into site/_site
make dist                        # build sdist + wheel (never publishes)
```

**The documentation site is `site/`** (Quarto → GitHub Pages, `.github/workflows/pages.yml`).
Its prose pages `{{< include >}}` the repository's own `docs/*.md` instead of copying them, so
the two cannot drift; `site/index.qmd` executes real ER2 on the `er2` kernel as it renders.
A relative link added to an included document must also be taught to `site/doc-links.lua`, or it
reaches the site as a dead path — `tests/test_site.py` fails when one is left unmapped.
