...
3. ✅ **Test macOS in CI**, so the README's platform table can move macOS from *experimental*
   ...
   **Done 2026-09-26:** the `test` job of `ci.yml` runs on `macos-latest` too, for 3.12–3.14.
   ...
   repositories), so the matrix now includes it. Its first run (`9d2f0a8`) was green on
   3.12–3.14, again 766 passed and 12 skipped, so the README lists macOS as supported on
   both, in one row.
