...
| `namespace` | available as `pari.<er2_name>` | 945 |
| `wrapper` | needs an ER2 wrapper (takes GP code; not a cypari2 method) | 30 |
...
| `Str` | `Str` | wrapper | type constructor (CapWords allowed by PEP 8); GP expression argument; ER2 takes a Python callable | concatenates its (string) argument into a single string |
...
| `eval` | `eval` | wrapper | Python builtin: never shadowed in the prelude; GP expression argument; ER2 takes a Python callable | evaluation of x, replacing variables by their value |
...
| `O` | `o` | wrapper | renamed for PEP 8; GP expression argument; ER2 takes a Python callable | p-adic or power series zero with precision given by e |
...
| `intcirc` | `intcirc` | wrapper | GP expression argument; ER2 takes a Python callable | numerical integration of expr on the circle \|z-a\|=R, divided by 2*I*Pi |
| `intfuncinit` | `intfuncinit` | wrapper | GP expression argument; ER2 takes a Python callable | initialize tables for integrations from a to b using a weight f(t) |
